...
const ResponseCard = ({ visible }) => {
  const response = window.serverData.response;
...
const ActionButtons = () => {
  ...
  const handleShare = () => {
    const tweetUrl = window.serverData.response.tweetUrl || 
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(window.serverData.response.question)}&url=${encodeURIComponent(window.location.href)}`;
